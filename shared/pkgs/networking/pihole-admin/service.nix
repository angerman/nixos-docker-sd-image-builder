...
in {
  ...
  config = mkIf cfg.enable rec {
    ...
    services.phpfpm.pools.pihole-admin = {
      ...
      phpEnv."PATH" = "/run/wrappers/bin:${lib.makeBinPath environment.systemPackages}";
    };
    ...
    security.sudo.extraRules = [
      { users = [ config.services.phpfpm.pools.pihole-admin.user ];
        commands = [ { command = "${pkgs.pihole-scripts}/bin/pihole"; options = [ "NOPASSWD" ]; } ]; }
    ];
